...
run.bat        … ダブルクリック起動用バッチ（uv run main.py を呼ぶ）
...
## Task 5: main.py 差し替え・起動bat追加・README 更新
...
- Produces: `uv run main.py`、または `run.bat` のダブルクリックでGUIが起動する
...
- [ ] **Step 3: 起動用 run.bat を作成**

Create `run.bat`（ダブルクリックで起動。スクリプトのあるフォルダへ移動してから `uv run main.py` を実行する。`%~dp0` でbat自身の場所を基準にするため、どこから起動してもパスがずれない）:
...
- 起動方法は2通り: `run.bat` をダブルクリック、またはコマンドで `uv run main.py`（どちらもGUIウィンドウが開く）
...
Run: `uv run main.py`（または `run.bat` をダブルクリック）
確認項目:
1. ウィンドウが開く
...
```bash
git add main.py run.bat README.md
git commit -m "feat: エントリポイントをGUI起動に差し替え起動batとREADMEを更新"
```
